...
[![RAG: 2,699 chunks](https://img.shields.io/badge/RAG%20corpus-2%2C699%20chunks-green.svg)](#layer-c--rag-코퍼스-3개)

---

대한민국 **개인정보보호법(PIPA)** 을 자연어로 검색·비교·분석·검증하는 MCP 입니다.

법제처 31개 + PIPC 공식 출처 인덱스 2개 + RAG 코퍼스 3개 + 환각 검증 1개 — 총 **37개 MCP 도구** 가 PIPA·시행령·PIPC 고시·의결례 + PIPC 공식 가이드 7종·상담사례 1,745건 (총 2,699 청크) 을 한 번에 다루면서 개인정보보호법과 관련된 문제해결을 도와줍니다.
...
- **개인정보 포털 가이드 및 상담사례 RAG 실무 데이터 강화**
  - 법제처 API 가 못 가진 실무 자료. 7종 가이드 954청크 (질의응답 99 + 소상공인 41 + CCTV 71 + 분야별 안내서 476 — 8개 편 전체 + 가명정보 처리 가이드라인 2026.3 본권·별권 132 + 개인정보 처리방침 작성지침 2026.4 96 + 공공 AX 프라이버시 보호 안내서 2026.7 39) + 개인정보 포털 상담사례 1,745건.
...
PIPC 가 발간한 공식 가이드 7종 + 개인정보 포털 상담사례 1,745건을 자연어로 검색합니다. 모든 답변에 PIPC 공식 출처가 첨부되어 검증 가능합니다.
...
→ 가이드 7종 + 상담사례를 통합 검색해 가장 관련성 높은 항목을 보여 줍니다.
```
...
| PIPC 가이드·상담사례 검색 | 3 | 가이드 7종 + 상담사례 1,745건 |
...
- **PIPC 공식 자료 RAG 코퍼스 2,699 청크** — 가이드 7종 (질의응답·소상공인 핸드북·CCTV 안내서·분야별 안내서 8개 편 전체·가명정보 처리 가이드라인·개인정보 처리방침 작성지침·공공 AX 프라이버시 보호 안내서) + 개인정보 포털 상담사례 1,745건. Contextual Retrieval 적용.
